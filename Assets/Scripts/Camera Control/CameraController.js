...
// The current global direction we want the camera to move in.
...
class cameraMotorMovement {
	...
	// How fast does the camera change speeds?  Higher is faster.
	var maxGroundAcceleration : float = 30.0;
	var maxAirAcceleration : float = 20.0;

	// The gravity for the camera
	...
	// We will keep track of the camera's current velocity,
	@System.NonSerialized
...
var movement : cameraMotorMovement = cameraMotorMovement();
...
class cameraMotorJumping {
	...
	// How much does the camera jump out perpendicular to the surface on walkable surfaces?
	// 0 means a fully vertical jump and 1 means fully perpendicular.
	var perpAmount : float = 0.0;
	
	// How much does the camera jump out perpendicular to the surface on too steep surfaces?
	// 0 means a fully vertical jump and 1 means fully perpendicular.
...
var jumping : cameraMotorJumping = cameraMotorJumping();

class cameraMotorMovingPlatform {
	var enabled : boolean = true;
...
var movingPlatform : cameraMotorMovingPlatform = cameraMotorMovingPlatform();

class cameraMotorSliding {
	// Does the camera slide on too steep surfaces?
	var enabled : boolean = true;
	
	// How fast does the camera slide on steep surfaces?
...
var sliding : cameraMotorSliding = cameraMotorSliding();
...
private function UpdateFunction () {
	...
	// This means our velocity will only be the amount the camera actually moved as a result of collisions.
	...
	// The cameraController can be moved in unwanted directions when colliding with things.
	...
	if (grounded && !IsGroundedTest()) {
		...
		// We pushed the camera down to ensure it would stay on the ground if there was any.
	...
	if (MoveWithPlatform()) {
		...
		// This works best when the camera is standing on moving tilting platforms. 
...
private function ApplyInputVelocityChange (velocity : Vector3) {	
	...
	if (grounded) {
		// When going uphill, the cameraController will automatically move up by the needed amount.
...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	if (grounded)
		...
		// Make sure we don't fall any faster than maxFallSpeed. This gives our camera a terminal velocity.
...
private function SubtractNewPlatformVelocity () {
	// When landing, subtract the velocity of the new ground from the camera's velocity
	...
	if (movingPlatform.enabled &&
		...
		// before we know the velocity of the platform under the camera
		if (movingPlatform.newPlatform) {
...
function CalculateJumpVerticalSpeed (targetJumpHeight : float) {
	...
	// for the camera to reach at the apex.
...
// Require a camera controller to be attached to the same game object
@script RequireComponent (CharacterController)
@script AddComponentMenu ("camera/camera Motor")
